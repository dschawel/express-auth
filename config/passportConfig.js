...
// Deserialize: Takes the user ID and looks up the rest of the information
...
passport.use(new GithubStrategy({
    ...
    .then(([user, wasCreated]) => {
        ...
        if (!wasCreated && user.githubId) {
            ...
            // Newly created user or, not a pervious GH user
            return cb(null, user)
        }
    })
    .catch(cb)
...
passport.use(new FacebookStrategy({
    ...
    let photo = profile.photos.length ? profile.photos[0].value : 
    ...
    .then(([user, wasCreated]) => {
        ...
        if (wasCreated  || user.facebookId) {
            // New user, not found in local databse
            cb(null, user)
        } else {
